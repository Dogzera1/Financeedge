...
function generateSignal(candles, symbol, timeframe = '1h', orderBook = null) {
  ...
  const ODDS = 2.0;
  const realEVData = probabilityCalibrator.getRealEV(signalForCalibrator, ODDS);

  let probability = realEVData.probability;
  let evPct = realEVData.evPct;
  let confidence = realEVData.confidence;
  let realEV = realEVData.realEV;

  // Fallback técnico: se calibrador neutro/baixo, usa score para gerar p
  if (confidence === 'BAIXA' && Math.abs(evPct) < 0.25) {
    const s = Math.max(2, Math.min(6, Math.abs(score))); // 2..6
    const pTech = 0.5 + (s - 2) * 0.02; // 0.50..0.58
    probability = parseFloat(pTech.toFixed(4));
    realEV = parseFloat(((probability * (ODDS - 1)) - (1 - probability)).toFixed(4));
    evPct = parseFloat((realEV * 100).toFixed(2));
    confidence = 'BAIXA';
    reasons.push('Fallback EV técnico (score)');
  }
