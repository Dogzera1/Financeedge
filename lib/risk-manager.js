...
const CONFIG = {
  ...
  // Permite fallback SL/TP % fixo quando ATR ausente
  allowFixedSlFallback: (process.env.ALLOW_FIXED_SL_FALLBACK || 'true').toLowerCase() !== 'false',
};
 ...
 */
function calcKelly({ evPct, odds, probability }, fraction = CONFIG.kellyFraction) {
  if (!odds || odds <= 1) return 0;
  const b = odds - 1;
  const p = (probability != null && probability > 0 && probability < 1)
    ? probability
    : (() => {
        const ev = (evPct || 0) / 100;
        if (!ev || ev <= 0) return null;
        // p = (EV + 1) / odds  (fallback quando só existe EV)
        return (ev + 1) / odds;
      })();
  ...
  if (!Number.isFinite(kellyFrac) || kellyFrac <= 0) return 0;
  const clamped = Math.min(CONFIG.maxStakePct, kellyFrac);
...
function calcStakeUsdtFromProbability(bankrollUsdt, probability, odds, confidenceLevel) {
  const fractionMap = { 'ALTA': 0.25, 'MÉDIA': 0.167, 'BAIXA': 0.10 };
  const fraction = fractionMap[confidenceLevel] || CONFIG.kellyFraction;
  const stakePct = calcKelly({ probability, odds }, fraction);
  const stakeUsdt = parseFloat((bankrollUsdt * stakePct).toFixed(2));
  return { stakeUsdt, stakePct, kellyFraction: fraction };
}
...
function calcStopTakeProfit(entryPrice, direction, atr, multiplierSL = 1.5, rrRatio = 2.0) {
  if (!atr || atr <= 0) {
    if (!CONFIG.allowFixedSlFallback) return null;
    // Fallback: % fixo
