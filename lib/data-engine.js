...
let exchangeHasCredentials = false;
...
function initExchange(name, options = {}) {
  try {
    ...
    exchangeName = name;
    exchangeHasCredentials = !!(options && options.apiKey && options.secret);
...
async function fetchTicker(symbol) {
  ...
  try {
    ...
    log('WARN', 'DATA', `fetchTicker ${symbol}: ${e.message}`);
    return null;
  }
}

async function fetchUSDTBRL() {
  const t = await fetchTicker('USDT/BRL');
...
function getExchangeStatus() {
  return {
    ...
    name: exchangeName,
    hasCredentials: exchangeHasCredentials,
  };
}
